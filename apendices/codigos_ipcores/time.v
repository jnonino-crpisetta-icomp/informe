...
// Date:              Wed Sep 12 20:32:03 2012 (by Create and Import 
//												Peripheral Wizard)
...
// Numero de Bits necesarios para representar el numero 
// cant_transiciones	
	parameter bits_cant_transiciones                = 4;	
// Tamano de los elementos de los arreglos, medido en bits	
...
// Tamano de los elementos de los arreglos de cuenta de tiempo, 
// medido en bits	
	parameter tamano_de_elementos_tiempo            = 32;	
  ...
  always @( slv_reg_read_sel )
    begin 
      case ( slv_reg_read_sel )
        ...
        //Usadas para leer
				32'b00000000000010000000000000000000 : slv_ip2bus_data <= disparos_en_espera;		//30 slv_reg11;
				32'b00000000000001000000000000000000 : slv_ip2bus_data <= disparos_posibles;		//34 slv_reg12;
				32'b00000000000000100000000000000000 : slv_ip2bus_data <= disparos_ya_ejecutados;	//38 slv_reg13;
				32'b00000000000000010000000000000000 : slv_ip2bus_data <= {{32-cant_transiciones{1'b0}},num_intr};//3C
				32'b00000000000000001000000000000000 : slv_ip2bus_data <= {{31{1'b0}},red_activa};//40
				32'b00000000000000000100000000000000 : slv_ip2bus_data <= marcado[0];//44		
				32'b00000000000000000010000000000000 : slv_ip2bus_data <= marcado[1];//48
				32'b00000000000000000001000000000000 : slv_ip2bus_data <= marcado[2];//4C
				32'b00000000000000000000100000000000 : slv_ip2bus_data <= marcado[3];//50
				32'b00000000000000000000010000000000 : slv_ip2bus_data <= marcado[4];//54
				32'b00000000000000000000001000000000 : slv_ip2bus_data <= marcado[5];//58
				32'b00000000000000000000000100000000 : slv_ip2bus_data <= marcado[6];//5C
				32'b00000000000000000000000010000000 : slv_ip2bus_data <= marcado[7];//60
				32'b00000000000000000000000001000000 : slv_ip2bus_data <= marcado[8];//64
				32'b00000000000000000000000000100000 : slv_ip2bus_data <= marcado[9];//68
				32'b00000000000000000000000000010000 : slv_ip2bus_data <= marcado[10];//6C
//				32'b00000000000000000000000000001000 : slv_ip2bus_data <= marcado[11];//70
//				32'b00000000000000000000000000000100 : slv_ip2bus_data <= marcado[12];//74
//				32'b00000000000000000000000000000010 : slv_ip2bus_data <= marcado[13];//78
//				32'b00000000000000000000000000000001 : slv_ip2bus_data <= marcado[14];//7C
...
/*REDES TEMPORALES*/		
	//Determinacion del vector de habilitacion de transiciones segun las "marcas temporales"
			for (k=0 ; k<cant_transiciones ; k=k+1) //Recorro transiciones
				begin
					if (vector_tiempo_sensibilizacion[k]>=vector_EFT[k] &
					vector_tiempo_sensibilizacion[k]<=vector_LFT[k]) t_enable_time[k] = 1'b1;
					else t_enable_time[k] = 1'b0;
				end
			
	//Determinacion de las transciones habilitadas, es decir, disparos posibles
				t_sensibilizadas = t_enable_sign & t_enable_inhibicion &
				 t_enable_limit;
	//disparos_posibles = t_sensibilizadas;								
				disparos_posibles = t_enable_sign & t_enable_inhibicion &
				 t_enable_limit & t_enable_time;
	...
	always @(*)
		begin
			for (indice_activa=0 ; indice_activa<cant_transiciones ; indice_activa=indice_activa+1)
				begin
					if (vector_tiempo_sensibilizacion[indice_activa] < vector_LFT[indice_activa])
					red_activa_reg[indice_activa] = 1'b1;
			...
			if(carga==1'b1)	//Proceso de carga
				...
				begin
					for(index2=0; index2<cant_transiciones; index2=index2+1)
						begin
							if ( colas_salida_llenas[index2]==1'b0 & 
							((transiciones_automaticas[index2]|
							 disparos_en_espera[index2]) &
							 disparos_posibles[index2] == 1'b1))
							  
				// Veo si el disparo en espera esta en los disparos posibles
								begin
						//Actualizo el marcado con el valor de la matriz resultado
										for (index3=0; index3<cant_plazas; 
										index3=index3+1)
											begin					
				...
				//Decremento el contador de la cola de disparos en espera correspondiente
										disparo_entrada_decrementa <= 
										 {cant_transiciones{1'b0}};
										if (disparos_en_espera[index2] == 1'b1)
				...
				//Reseteo la marca de tiempo de la transicion que se disparo
								...
								begin
									...
									//Actualizo marcas temporales
										if (t_sensibilizadas[index2] == 1'b0)
										 vector_tiempo_sensibilizacion[index2] <=
										  {tamano_de_elementos_tiempo{1'b0}};
										if (t_sensibilizadas[index2] == 1'b1) 
									//Transicion sensibilizada
											begin
												if (vector_tiempo_sensibilizacion[index2]+vector_incremento_de_tiempo[index2] >=
												 {tamano_de_elementos_tiempo{1'b1}} )
													...
													begin
														vector_tiempo_sensibilizacion[index2] <=
														 vector_tiempo_sensibilizacion[index2]+vector_incremento_de_tiempo[index2];
													end
											end
	...
	always@(negedge Bus2IP_Clk)
		begin
			if (Bus2IP_Resetn==1'b0)
				...
				begin
					Interrupt_reg<=
					{Interrupt_reg[0],(|(disparos_ya_ejecutados & intr_mask))&!intr_reset};
...
//BUS (se debe cumplir: tamano_bus=bits_bus_plazas+bits_bus_transiciones+bits_bus_elemento)
	...
	//Cantidad de bits del dato del bus dedicado a indicar la fila de la matriz			
		localparam bits_bus_plazas			= 8;
	//Cantidad de bits del dato del bus dedicado a indicar la columna de la matriz
	...
	always@(negedge Bus2IP_Clk, negedge Bus2IP_Resetn)
			...
			if (Bus2IP_Resetn==1'b0)
				begin//Reseteo de variables
					...
					filas_reset=filas_reset+1)
						begin
						marcado_inicial[filas_reset]<= 
						 {tamano_de_elementos{1'b0}};
						cotas_plazas [filas_reset]<=
						 {tamano_de_elementos{1'b0}};
							for (columnas_reset=0; columnas_reset<cant_transiciones;
							 columnas_reset=columnas_reset+1)
								begin
									matriz_incidencia[filas_reset][columnas_reset] 	<=
									 {tamano_de_elementos{1'b0}};
									matriz_inhibicion [filas_reset][columnas_reset] <=
									 1'b0;
									vector_EFT [columnas_reset]<=
									{tamano_de_elementos_tiempo{1'b0}};//Vector tiempo minimo
									vector_LFT [columnas_reset]<=
									{tamano_de_elementos_tiempo{1'b1}};//Vector tiempo maximo
									vector_incremento_de_tiempo [columnas_reset]<=
									{tamano_vector_incremento_de_tiempo{1'b0}};
								end //FIN for (filas_reset=0; filas_reset<cant_plazas; 
								filas_reset=filas_reset+1)
		...
		load_vector_EFT:
			begin
				carga<=1'b1;
				intr_reset<=0;
				if (Bus2IP_Data[tamano_bus-1:tamano_bus-bits_bus_plazas]==8'b00000000) 
					//Parte menos significativa de duracion
				begin
							vector_EFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:
				tamano_bus-bits_bus_plazas-bits_bus_transiciones]][15:0]<=Bus2IP_Data[bits_bus_elementos-1:0];
					end
				else if (Bus2IP_Data[tamano_bus-1:tamano_bus-bits_bus_plazas]==8'b00000001)
					begin
					vector_EFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]][31:16]<=
						Bus2IP_Data[bits_bus_elementos-1:0];
					end
//				else if (Bus2IP_Data[tamano_bus-1:tamano_bus-bits_bus_plazas]==8'b00000010)//Parte mas significativa de duracion
//					begin
//						vector_EFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]][47:32]<=Bus2IP_Data[bits_bus_elementos-1:0];
//					end
//				else if (Bus2IP_Data[tamano_bus-1:tamano_bus-bits_bus_plazas]==8'b00000011)//Parte mas significativa de duracion
//					begin
//						vector_EFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]][63:48]<=Bus2IP_Data[bits_bus_elementos-1:0];
//					end
//				else if (Bus2IP_Data[tamano_bus-1:tamano_bus-bits_bus_plazas]==8'b00000100)//Parte mas significativa de duracion
//						begin
//						vector_EFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]][79:64]<=Bus2IP_Data[bits_bus_elementos-1:0];
//					end
//				else if (Bus2IP_Data[tamano_bus-1:tamano_bus-bits_bus_plazas]==8'b00000101)//Parte mas significativa de duracion
//					begin
//							vector_EFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]][95:80]<=Bus2IP_Data[bits_bus_elementos-1:0];
//				end
//				else if (Bus2IP_Data[tamano_bus-1:tamano_bus-bits_bus_plazas]==8'b00000110)//Parte mas significativa de duracion
//					begin
//						vector_EFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]][111:81]<=Bus2IP_Data[bits_bus_elementos-1:0];
//					end
//				else if (Bus2IP_Data[tamano_bus-1:tamano_bus-bits_bus_plazas]==8'b00000111)//Parte mas significativa de duracion
//					begin
//						vector_EFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]][127:112]<=Bus2IP_Data[bits_bus_elementos-1:0];
//					end
				disparo_entrada_incrementa <= {cant_transiciones{1'b0}};
				disparo_salida_decrementa <= {cant_transiciones{1'b0}};
				ack_escritura_reg = 1'b1;
		...
		load_vector_incrementos_tiempo:
				begin
					...
					vector_incremento_de_tiempo 
					[Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]]<=
					Bus2IP_Data[bits_bus_elementos-1:0];//generar vector sacar de aca
					disparo_entrada_incrementa <= {cant_transiciones{1'b0}};
				disparo_salida_decrementa <= {cant_transiciones{1'b0}};
					ack_escritura_reg = 1'b1;
				end
			load_vector_LFT:
				begin
					...
					8'b00000000)//Parte menos significativa de duracion 
						...
						begin
							vector_LFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]]
...
//							begin
//								vector_LFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]]
//								[9:80]<=Bus2IP_Data[bits_bus_elementos-1:0];
//							end
//						else if (Bus2IP_Data[tamano_bus-1:tamano_bus-bits_bus_plazas]==8'b00000110)//Parte mas significativa de duracion
//							begin
//								vector_LFT [Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]]
...
//							end
				...
				if (colas_salida_vacias[Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-bits_bus_plazas-bits_bus_transiciones]]==1'b0 ) // La cola del disparo que se quiere sacar no esta vacia
					begin
						...
						disparo_entrada_incrementa <={cant_transiciones{1'b0}};
						ack_escritura_reg = 1'b1;
						if(intr_mask[Bus2IP_Data[tamano_bus-bits_bus_plazas-1:tamano_bus-
